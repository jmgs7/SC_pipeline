...
plot2 <- FeatureScatter(run2.raw, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
```
...
run2.subset <- subset(run2.raw, Sample_Tag != "Undetermined" & Sample_Tag != "Multiplet")
...
run2.subset$Sample_Tag_Number <- as.integer(gsub("\\D", "", run2.subset$Sample_Tag))

# Extract Sample Name and Sample Group based on Sample Tag information
run2.subset$Sample_Name <- filtered_excel_data$Sample_Name[match(run2.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run2.subset$Sample_Group <- filtered_excel_data$Sample_Group[match(run2.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run2.subset$Sample_Name_Group <- filtered_excel_data$Sample_Name_Group[match(run2.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]
```
...
run2.subset <- SCTransform(run2.subset, vst.flavor = "v2", vars.to.regress = "percent.mt")
```
...
plot2 <- FeatureScatter(run2.subset, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
```
...
run2.subset <- RunPCA(run2.subset, assay = "SCT") # features = VariableFeatures(object = run2.subset))

DimPlot(run2.subset, reduction = "pca") + NoLegend() # An unique PCA is not enough to separate cell groups.
...
DimHeatmap(run2.subset, dims = 1:30, balanced = TRUE)
```

```{r PCA_top_features, fig.height=30, fig.width=15}
VizDimLoadings(run2.subset, dims = 1:30, reduction = "pca")
```
...
run2.subset <- FindNeighbors(run2.subset, dims = 1:30)
run2.subset <- FindClusters(run2.subset, resolution = 0.8) # Default resolution = 0.8
run2.subset <- RunUMAP(run2.subset, dims = 1:30)
```

```{r cluster_visualization, fig.height=6, fig.width=10}
DimPlot(run2.subset, reduction = "umap", label = TRUE)
```
...
# Visualize QC metrics as a violin plot
VlnPlot(run2.subset, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) + NoLegend()

# Zoom in on nFeature_RNA violin plot.
VlnPlot(run2.subset, features = "nFeature_RNA", ncol = 1) + ylim(0, 2500) + NoLegend()
...
run2.SCexp <- as.SingleCellExperiment(run2.subset)
...
run2.subset[["cell.labels"]] <- SingleR.annotation$labels
```

```{r annotation_vis, fig.height=6, fig.width=10}
DimPlot(run2.subset, reduction = "umap", group.by = "cell.labels", label = TRUE, repel = TRUE, label.size = 4) + theme(axis.title = element_text(size = 15), legend.text = element_text(size = 10), axis.text = element_text(size = 10)) + guides(colour = guide_legend(override.aes = list(size = 3)))
```
...
```{r}
clusters <- run2.subset@meta.data[["seurat_clusters"]]
...
run2.subset[["cluster.labels"]] <- SingleR.annotation$labels
```
...
cluster_number <- levels(run2.subset[["seurat_clusters"]][1, ])
...
names(new.clusters.ids) <- levels(run2.subset)
run2.subset <- RenameIdents(run2.subset, new.clusters.ids)
```

```{r cluster_annotation_vis, fig.height=6, fig.width=10}
DimPlot(run2.subset, reduction = "umap", label = TRUE, repel = TRUE, label.size = 4) + theme(axis.title = element_text(size = 15), legend.text = element_text(size = 10), axis.text = element_text(size = 10)) + guides(colour = guide_legend(override.aes = list(size = 3)))
```

```{r saving_data, include = FALSE, echo = FALSE}

# Save fully-proccesed data.
saveRDS(run2.subset, file = "~/Documents/SC_Prostate/Output/run2/run2_processed.rds")
```
