...
run2.subset <- NormalizeData(run2.subset, scale.factor = 1e6, assay = "RNA") # scale.factor = 1e6 means we are using CPMs.
run2.subset <- FindVariableFeatures(run2.subset, nfeatures = 3000, assay = "RNA")
run2.subset <- ScaleData(run2.subset, assay = "RNA", vars.to.regress = "percent.mt") # regress out % mito.
```
...
run2.subset <- RunPCA(run2.subset, assay = "RNA") # features = VariableFeatures(object = run2.subset))
...
DimHeatmap(run2.subset, dims = 1:20, balanced = TRUE)
```

```{r PCA_top_features, fig.height=30, fig.width=15}

VizDimLoadings(run2.subset, dims = 1:20, reduction = "pca")
```
...
run2.subset <- FindNeighbors(run2.subset, dims = 1:20)
run2.subset <- FindClusters(run2.subset, resolution = 1) # Default resolution = 0.8
run2.subset <- RunUMAP(run2.subset, dims = 1:20)
```
...
SingleR.annotation <- SingleR(test = run2.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, num.threads = parallel::detectCores())
...
SingleR.annotation <- SingleR(test = run2.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, clusters = clusters, num.threads = parallel::detectCores())
