...
test <- Reduce(function(x,y) merge(x,y,add.cell.ids = c(x@project.name,y@project.name)) , rds_objects)
...
print(dim(test)) # Merged object should have 13420 cells.
...
test <- 
  SCTransform(test, vst.flavor = "v2", vars.to.regress = c("percent.mt","orig.ident")) %>% 
  RunPCA(assay = "SCT") %>% 
...
test <- readRDS("~/Documents/SC_Prostate/Data/run2/C2_expected.rds") %>%
  PercentageFeatureSet(pattern = "^MT-", col.name = 'percent.mt') %>% 
  subset(Sample_Tag != "Undetermined" & Sample_Tag != "Multiplet")
...
if (min(test$nCount_RNA) >= minCov) {
  countLOW <- min(test$nCount_RNA)
} else {
  countLOW <- quantile(test$nCount_RNA, prob = c(0.01))
}
countHIGH <- quantile(test$nCount_RNA, prob = 0.99)
featureHIGH <- quantile(test$nFeature_RNA, prob = 0.99)
featureLOW <- quantile(test$nFeature_RNA, prob = 0.01)
