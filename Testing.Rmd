...
for(i in 1:length(clusters)){
  if (clusters[i] != "4" & clusters[i] != "15") {
      cur_df <- as.data.frame(SCP_data@meta.data %>% subset(seurat_clustect with cell metadata (colData) columnsrs == clusters[i]) %>% .$orig.ident %>% table() /
...
It is necessary to normalize the data before integration. Accordin to mantainers (https://github.com/satijalab/seurat/issues/4811) it doesn't mind to normalize before or after splitting.
