...
# subset
run1.subset <- subset(run1.raw, subset = nFeature_RNA > featureLOW & nFeature_RNA < featureHIGH & nCount_RNA > countLOW  & nCount_RNA < countHIGH & percent.mt < 25 & Sample_Tag != "Undetermined" & Sample_Name != "Multiplet")
...
run1.subset <- FindClusters(run1.subset, resolution = 1.2) # Default resolution = 0.8
...
singler.annotation <- SingleR(test = run1.subset.SCexp, ref = reference, assay.type.test=1,
    labels = reference$label.main)

run1.subset[["SingleR.labels"]] <- singler.annotation$labels
```
...
run1 <- readRDS("../Data/run1/C1_Seurat_Edu.rds") %>% PercentageFeatureSet(pattern = "^MT-", col.name = 'percent.mt') %>% SCTransform(vst.flavor = "v2", vars.to.regress = 'percent.mt') %>% RunPCA(assay = "SCT") %>% FindNeighbors(dims = 1:30) %>% RunUMAP(dims = 1:30) %>% FindClusters(resolution = 1.2)
```
