...
new.clusters.ids <- SingleR.annotation$labels
...
run1 <- readRDS("../Data/run1/C1_Seurat_Edu.rds") %>% PercentageFeatureSet(pattern = "^MT-", col.name = 'percent.mt') %>% SCTransform(vst.flavor = "v2", vars.to.regress = 'percent.mt') %>% RunPCA(assay = "SCT") %>% FindNeighbors(dims = 1:30) %>% FindClusters(resolution = 1.2) %>% RunUMAP(dims = 1:30)
```
