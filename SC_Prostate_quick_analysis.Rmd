...
library(sctransform)
library(ggplot2)
library(DoubletFinder) # Remove multiplets.
...
VlnPlot(run1.raw, features = c("nFeature_RNA", "nCount_RNA"), ncol = 2)
...
run1 <- readRDS("../Data/run1/C1_Seurat_Edu.rds") %>% PercentageFeatureSet(pattern = "^MT-", col.name = 'percent.mt') %>% SCTransform(vst.flavor = "v2", vars.to.regress = 'percent.mt') %>% RunPCA(assay = "SCT") %>% FindNeighbors(dims = 1:30) %>% RunUMAP(dims = 1:30) %>% FindClusters()
```
