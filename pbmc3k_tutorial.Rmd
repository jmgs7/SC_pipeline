...
knitr::opts_knit$set(root.dir = "~/Documents/Seurat_practise/pbmc3k_filtered_gene_bc_matrices/")
```
...
pbmc.data <- Read10X(data.dir = "~/Documents/Seurat_practises/pbmc3k_filtered_gene_bc_matrices/hg19")
# This gets number of UMIs (rows) and number of samples (columns).
dim(pbmc.data)


# Initialize the Seurat object with the raw (non-normalized data).
pbmc <- CreateSeuratObject(counts = pbmc.data, project = "pbmc3k", min.cells = 3, min.features = 200) # keeping genes expressed in >= 3 cells and cells with at least 200 detected genes

pbmc #general information, a Seurat object contains assays: object[[slots]] to store different information, new assays will be added during analysis
pbmc[["RNA"]] #as our active assay is "RNA"
dim(pbmc[["RNA"]])#so we are keeping 13714 genes
pbmc[["RNA"]] [1:5,1:5] #shows first 5 rows and columns data for read counts
pbmc[["RNA"]]$counts[1:5,1:5] #same as previous as we do not have info for rest
#of @slots in pbmc[["RNA"]]
#colnames(pbmc[["RNA"]]) or
#colnames(pbmc[["RNA"]]@counts) #shows headings, similarly, functions for data frames and matrices can be used

pbmc@meta.data [1:5,] # A Seurat Object has metadata information including
#nCount_RNA which is the total number of read counts per cell
#nFeature_RNA which is the total number of expressed UMIs or genes per cell
...
#Convert data to a regular matrix and save it as a table
x = as.matrix(x = pbmc.data)
write.csv(x,"pbmc.csv")
#write.table(x,"pbmc.txt")

#Read data fromt table and convert it to a Sparse Matrix
table <- read.csv("pbmc.csv",row.names=1)
#table <- read.table("pbmc.txt",header=TRUE)
m <-as.matrix(table)
pbmc.data <- Matrix(data=m, sparse=TRUE)
m[1:5,1:5]
pbmc.data[1:5,1:5]
...
sparse.size
dense.size / sparse.size # Ocupa 24 veces menos espacio.
```
...
head(colnames(pbmc[["RNA"]]), 10) # Cells are identified by their UMIs
head(colnames(pbmc[["RNA"]]$counts), 10) #shows headings, similarly, functions for data frames and matrices can be used
...
# Extract mitochondrial genes using 'grep' function, grep grep searches for matches to argument pattern
# here we need to extract any gene starting with 'MT'
mito.genes <- grep(pattern = "^MT-", x = rownames(x = pbmc[["RNA"]]), value = TRUE) 
# How many mitochondrial genes with reads are there in our cell pool?
length(mito.genes)

# Extract percent of mitochondrial genes and put them in Seurat metadata manually
proportion.mito.reads <- colSums(pbmc[["RNA"]][c(mito.genes),])/colSums(pbmc[["RNA"]])
pbmc <- AddMetaData(object = pbmc, metadata = proportion.mito.reads, col.name = "proportion.mt.reads")
pbmc[["mito.genes"]]<-proportion.mito.reads
pbmc@meta.data[1:5,]
```
...
## What is the number on top of the graph?

R\^2 value (Recommended R\^2\>0.95) it tells you about correlation by looking at nFeatures (genes) vs nCounts (reads, UMIs) graph.

## Is there a specific behaviour for certain cells?

There are several cells with unusual number of expressed genes/UMIs over 2500, which can be filtered out

## What would be the expectation for mitochondrial gene expression?

Mitochondrial genes should be expressed at a constant rate, we detect several cells which abnormally high percentage of mitochondrial genes, highly expressed and with very few number of UMIs –\> Cell death,

We filter cells with more than 2500 detected number and genes and more than 5 % of mitochondrial genes. 200 features indicate that they are damaged cells with very low quality. More than 1500 counts indicate that it could be more than 1 cell in a drop of the drop-seq. THIS PARAMETERS DEPENS ON YOUR DATASET.
...
plot1 + plot2
# Also CombinePlots(plots = list(plot1, plot2)
...
```{r normalize.visualization}

# Visualize effects of normalizalition in total cell expression
Normalized.Reads <- colSums(pbmc[["RNA"]]$data)
pbmc <- AddMetaData (pbmc,Normalized.Reads,col.name="Normalized.Reads")
VlnPlot(object = pbmc, features= c("nCount_RNA", "Normalized.Reads"), ncol = 2)
```

```{r nomalize.visualizatoin.mt}

# Visualize effects of normalization in mitochondrial gene expression percentage
proportion.mito.reads.N <- colSums(pbmc[["RNA"]]$data[c(mito.genes),])/colSums(pbmc[["RNA"]]$data)
pbmc <- AddMetaData(object = pbmc, metadata = proportion.mito.reads.N, col.name = "proportion.mt.reads.N")
pbmc@meta.data[1:5,]
plot1 <- FeatureScatter(pbmc, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(pbmc, feature1 = "nFeature_RNA", feature2 = "percent.mt")
plot3 <- FeatureScatter(pbmc, feature1 = "nCount_RNA", feature2 = "proportion.mt.reads.N")
plot4 <- FeatureScatter(pbmc, feature1 = "nFeature_RNA", feature2 = "proportion.mt.reads.N")
CombinePlots(plots = list(plot1, plot2, plot3, plot4))

# Basically % mito is now normally distributed (take nCount_RNA and nFeature_RNA as y axis), so has more of a "constant aspect".
```
...
# Scaling the data and removing unwanted variation.
...
# What is does if it calculates an score for the variation source you want to delete and filters in function of this score.
...
DimPlot(pbmc, reduction = 'pca') + NoLegend() # An unique PCA is not enough to separate cell groups.
```
...
# Seurat cluster cells based on PCA scores, with each component essentially representing a ?metagene?
# that combines information across a correlated gene set.
# It is imporant to determine how many components to use. Seurat calculates signifcance of components
# by randomly permute a subset of the data (1% by default) and reruning PCA, thus constructing a ?null
# distribution? of gene scores though resampling in a test inspired by the jackStraw procedure
pbmc <- JackStraw(pbmc, num.replicate = 100)
pbmc <- ScoreJackStraw(pbmc, dims = 1:20)

# Significant components are those who have a  srong enrichment of low p-values
# Genes with significant p-values are dots over dashed line
# In this case it appears that there is a sharp drop-off in significance after the 12th PC.
# Is recommended to err on the higher side, so I will be recommendable to pick till the 13th PC.
...
# Para tipos celulares generales te vale resolucioens bajas.
# Para subtipos y tipos más concretos resoluciones mayores o clustering de clusters.
...
Re-run the clustering using first 10 PCs for testing. As you can see, it may change the distribution of the group across the plot, but the groups' number and composition doesn't seem affected.

```{r cluster_10_dims, fig.height=8, fig.width=8}
...
Seurat can help you find markers that define clusters via differential expression (DE). By default, it identifies positive and negative markers of a single cluster (specified in `ident.1`), compared to all other cells. `min.pct` argument requires markers to be detected at a minimum percentage in rest of cells, and the `thresh.test` argument requires markers to be differentially expressed (on average).`FindAllMarkers()` automates this process for all clusters, but you can also test groups of clusters vs. each other, or against all cells.
