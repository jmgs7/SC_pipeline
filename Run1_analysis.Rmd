...
run1.subset <- NormalizeData(run1.subset, scale.factor = 1e6, assay = "RNA") # scale.factor = 1e6 means we are using CPMs.
run1.subset <- FindVariableFeatures(run1.subset, nfeatures = 3000, assay = "RNA")
run1.subset <- ScaleData(run1.subset, assay = "RNA", vars.to.regress = "percent.mt") # regress out % mito.
```
...
run1.subset <- RunPCA(run1.subset, assay = "RNA") # features = VariableFeatures(object = run2.subset))
...
run1.subset <- FindClusters(run1.subset, resolution = 1) # Default resolution = 0.8
...
SingleR.annotation <- SingleR(test = run1.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, num.threads = parallel::detectCores())
...
SingleR.annotation <- SingleR(test = run1.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, clusters = clusters, num.threads = parallel::detectCores())
