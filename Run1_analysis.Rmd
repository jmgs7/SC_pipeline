...
UPDATE: Since we know how to introduced the missing information into the raw matrix, we will use our own analysis form now on.
...
plot2 <- FeatureScatterGradient(run1.raw, feature1 = "nCount_RNA", feature2 = "nFeature_RNA", gradient = "percent.mt", upper.limit = 100)
plot1 + plot2
```
...
run1.subset <- subset(run1.raw, Sample_Tag != "Undetermined" & Sample_Tag != "Multiplet")
...
We need to perform an additional filtering step to remove a group of low feature and read count cells.
...
### 2.5. Adding sample names and sample groups.
...
run1.subset$Sample_Tag_Number <- as.integer(gsub("\\D", "", run1.subset$Sample_Tag))

# Extract Sample Name and Sample Group based on Sample Tag information
run1.subset$Sample_Name <- filtered_excel_data$Sample_Name[match(run1.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run1.subset$Sample_Group <- filtered_excel_data$Sample_Group[match(run1.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run1.subset$Sample_Name_Group <- filtered_excel_data$Sample_Name_Group[match(run1.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]
```
...
### 2.6. Normalization using SCTransform.
...
run1.subset <- SCTransform(run1.subset, vst.flavor = "v2", vars.to.regress = "percent.mt", seed.use = NULL)
```
...
run1.subset <- RunPCA(run1.subset, assay = "SCT", seed.use = NULL) # features = VariableFeatures(object = run1.subset))
...
run1.subset <- RunUMAP(run1.subset, dims = 1:30, seed.use = NULL)
```
