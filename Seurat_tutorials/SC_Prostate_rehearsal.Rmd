...
Import raw, preprocessed data.

```{r import_raw_data_preprocessed}
# run1.subset <- readRDS("~/Documents/SC_Prostate/Output/Run1_raw.rds")
# run1.subset
```
...
# # Transform the .h5 file output to a file compatible with Seurat.
...
saveRDS(run1.subset, file = "../Output/Run1_raw.rds")
```
...
SingleR.annotation <- SingleR(test = run1.subset.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, num.threads = detectCores())
...
SingleR.annotation <- SingleR(test = run1.subset.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, clusters = clusters, num.threads = detectCores())
