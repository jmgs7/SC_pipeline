...
# run sctransform
pbmc <- SCTransform(pbmc, vst.flavor = "v2", vars.to.regress = "percent.mt", verbose = TRUE)
```
...
pbmc <- LoadData("pbmc3k") %>% PercentageFeatureSet(pattern = "^MT-",col.name = 'percent.mt') %>% SCTransform(vars.to.regress = 'percent.mt') %>%
