...
SCDiffExp <- function(SeuratObject, mode, test, control, fc.threshold = 1.5, p.threshold = 0.05) {
  ...
  DE <- FindMarkers(SeuratObject, ident.1 = test, ident.2 = control, group.by = group.by)
...
wilcoxauc(data_matrix, y = colnames(data_matrix))
```
...
y <- DE_T
  ...
  overlap_names <- intersect(x_names, y_names)
  
  # Step 3: Determine direction of differential expression
  x_up <- names(x[x > 0])
  x_down <- names(x[x < 0])
  y_up <- names(y[y > 0])
  y_down <- names(y[y < 0])
