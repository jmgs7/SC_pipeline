...
SCDiffExp <- function(SeuratObject, mode, test, control, fc.threshold = 1.5, p.threshold = 0.05) {
  ...
  if (mode == "sample") {
    ...
    recorrect_umi <- FALSE
      ...
      print("DE analysis can only be executed by sample or by group")
      return(NULL)
    }
...
SCDiffExp.sample <- function(SeuratObject, mode, test, control, fc.threshold = 1.5, p.threshold = 0.05) {
   ...
   if (mode == "sample") {
    ...
    for (sample in unique(DE_data$Sample_Name)) {
     ...
     print("This function only executes DE by sample")
     return(NULL)
  }
}
...
SimilariyScoreDiffExp <- function(SeuratObject, mode, target, reference, control, fc.threshold = 1.5, p.threshold = 0.05, method = "jaccard") {
  
  if (method != "jaccard" & method != "cosine") {
    print("Only Jaccard Index and Cosine Similarity methods available for DiffExp-based similarity")
    return(NULL)
  } else {
    ...
    if (mode == "sample") {
      ...
      DE_reference <- SCDiffExp.sample(SeuratObject = SeuratObject, mode = mode, test = reference, control = control, fc.threshold = fc.threshold, p.threshold = p.threshold)
      
      # For security, intersect the names of the samples and only estimate the similarity score of the common samples.
      samples <- intersect(names(DE_target), names(DE_reference))
      
      ##  TODO: Create the loop to calculate the similarity scores between samples, review ALL the code.
...
# Calculate average expression of the control group.
ctrl_group <- subset(SCP_data, Sample_Group == "N")
ctrl_group$Sample_Name <- ctrl_group$Sample_Group
# ctrl_group_avg <- AverageExpression(ctrl_group, assay = "SCT", slot = "scale.data")
# ctrl_group_avg <- ctrl_group_avg$SCT[,1]
# data_matrix[["control"]] <- ctrl_group_avg
# Get target cells.
target_cells <- subset(SCP_data, Sample_Group == "AT")

DE_data <- merge(ctrl_group, target_cells)
#DE <- PrepSCTFindMarkers(DE_data, assay = "SCT")
DE <- FindMarkers(DE_data, ident.1 = "CaPBi-23-20-S", ident.2 = "N", group.by = "Sample_Name", recorrect_umi = FALSE, assay = "SCT")

DE_values <- SCDiffExp(DE_data, mode = "sample", test = "CaPBi-23-20-S", control = "N")
DE_values

# # Loop over each unique sample name to calculate similarity scores
# for (sample in unique(target_cells$Sample_Name)) {
#   
#     DE <- FindMarkers(DE_data, ident.1 = sample, ident.2 = "N", group.by = "Sample_Name", recorrect_umi = FALSE, assay = "SCT")
...
#     data_matrix[[sample]] <- sample_avg_expression
# 
# }
# 
# data_matrix <- do.call(cbind, data_matrix)
```
