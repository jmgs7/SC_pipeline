...
SCDiffExp.cluster <- function(SeuratObject, mode = "cluster", test, control, fc.threshold = 1.5, p.threshold = 0.05) {
  # NOTE: DE = Differential Expression
  ...
  for (cluster in levels(SeuratObject$seurat_clusters)) {
    ...
    DE[[cluster]] <- SCDiffExp(SeuratObject = DE_data, mode = mode, test = test, control = control, fc.threshold = fc.threshold, p.threshold = p.threshold)
  }
...
ctrl_clusters <- list()
test_clusters <- list()
for (cluster in levels(SCP_data$seurat_clusters)) {
    
      # Step 1: subset the control group. For convenience, the sample group is added as the sample name for all cells. That way all the control group will be treated as a single sample in further steps.
    ctrl_group <- subset(SCP_data, Sample_Group == "N" & seurat_clusters == cluster)
    # Step 2: Subset the test group.
    test_cells <- subset(SCP_data, Sample_Group == "T" & seurat_clusters == cluster)
    
    ctrl_clusters[cluster] <- ctrl_group
    test_clusters[cluster] <- test_cells
}
...
similarity_scores_AT <- SCsimilarity(SCP_data, mode = "cluster", method = "jaccard", target = "AT", reference = "T", control = "N", use.diffexp = TRUE)

similarity_scores_AT

## TODO 
## - Use normal p-value for clusters for better significance.
## - Ident mode.
## - Review code.
```
...
```{r SimilarityScoreDiffExp, include = FALSE}

SimilarityScoreDiffExp <- function(SeuratObject, mode, target, reference, control, method = "jaccard", fc.threshold = 1.5, p.threshold = 0.05) {
  # NOTE: DE = Differential Expression.
  ...
  if (method != "jaccard" & method != "cosine") {
    ...
    if (mode == "sample") {
      ...
      DE_target <- SCDiffExp.cluster(SeuratObject = SeuratObject, mode = mode, test = target, control = control, fc.threshold = fc.threshold, p.threshold = p.threshold)
      DE_reference <- SCDiffExp.cluster(SeuratObject = SeuratObject, mode = mode, test = reference, control = control, fc.threshold = fc.threshold, p.threshold = p.threshold)
...
SCsimilarity <- function(
  ...
  if (use.diffexp) {
    ...
    # For security if the user set both options to TRUE.
    use.top.genes = FALSE
  ...
  if (mode == "cluster") {
    ...
    for (cluster in levels(SCP_data$seurat_clusters)) {
      ...
      ref_cells <- subset(SeuratObject, Sample_Group == reference, serurat_clusters == cluster)
      ...
      target_cells <- subset(SeuratObject, Sample_Group == target, serurat_clusters == cluster)
