---
title: "Clustering with resolution = 0.4"
...
This document is an annex to SC_Prostate_analysis markdown showing the clustering process using a resolution value of 0.5.
...
SCP_data <- SetIdent(SCP_data, value = SCP_data$RNA_snn_res.0.4)
```
...
clusters <- SCP_data@meta.data[["RNA_snn_res.0.4"]]
...
SCP_data[["cluster.labels_res=0.4"]] <- SingleR.annotation$labels
```
...
cluster_number <- levels(SCP_data[["RNA_snn_res.0.4"]][1, ])
...
SCP_data[["cell.cluster.labels_res=0.4"]] <- Idents(SCP_data)
```
