...
run3.subset <- NormalizeData(run3.subset, scale.factor = 1e6, assay = "RNA") # scale.factor = 1e6 means we are using CPMs.
run3.subset <- FindVariableFeatures(run3.subset, nfeatures = 3000, assay = "RNA")
run3.subset <- ScaleData(run3.subset, assay = "RNA", vars.to.regress = "percent.mt") # regress out % mito.
```
...
run3.subset <- RunPCA(run3.subset, assay = "RNA") # features = VariableFeatures(object = run3.subset))
...
DimHeatmap(run3.subset, dims = 1:20, balanced = TRUE)
```

```{r PCA_top_features, fig.height=30, fig.width=15}

VizDimLoadings(run3.subset, dims = 1:20, reduction = "pca")
```
...
run3.subset <- FindNeighbors(run3.subset, dims = 1:20)
run3.subset <- FindClusters(run3.subset, resolution = 1) # Default resolution = 0.8
run3.subset <- RunUMAP(run3.subset, dims = 1:20)
```
...
SingleR.annotation <- SingleR(test = run3.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, num.threads = parallel::detectCores())
...
SingleR.annotation <- SingleR(test = run3.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, clusters = clusters, num.threads = parallel::detectCores())
