...
plot2 <- FeatureScatter(run3.raw, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
```
...
run3.subset <- subset(run3.raw, Sample_Tag != "Undetermined" & Sample_Tag != "Multiplet")
...
run3.subset$Sample_Tag_Number <- as.integer(gsub("\\D", "", run3.subset$Sample_Tag))

# Extract Sample Name and Sample Group based on Sample Tag information
run3.subset$Sample_Name <- filtered_excel_data$Sample_Name[match(run3.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run3.subset$Sample_Group <- filtered_excel_data$Sample_Group[match(run3.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]

run3.subset$Sample_Name_Group <- filtered_excel_data$Sample_Name_Group[match(run3.subset$Sample_Tag_Number, filtered_excel_data$Sample_Tag)]
```
...
run3.subset <- SCTransform(run3.subset, vst.flavor = "v2", vars.to.regress = "percent.mt")
```
...
plot2 <- FeatureScatter(run3.subset, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")
plot1 + plot2
```
...
run3.subset <- RunPCA(run3.subset, assay = "SCT") # features = VariableFeatures(object = run3.subset))

DimPlot(run3.subset, reduction = "pca") + NoLegend() # An unique PCA is not enough to separate cell groups.

ElbowPlot(run3.subset, ndims = 50)
```
...
DimHeatmap(run3.subset, dims = 1:30, balanced = TRUE)
```

```{r PCA_top_features, fig.height=30, fig.width=15}
VizDimLoadings(run3.subset, dims = 1:30, reduction = "pca")
```
...
run3.subset <- FindNeighbors(run3.subset, dims = 1:30)
run3.subset <- FindClusters(run3.subset, resolution = 0.8) # Default resolution = 0.8
run3.subset <- RunUMAP(run3.subset, dims = 1:30)
```

```{r cluster_visualization, fig.height=6, fig.width=10}
DimPlot(run3.subset, reduction = "umap", label = TRUE)
```
...
# Visualize QC metrics as a violin plot
VlnPlot(run3.subset, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3) + NoLegend()

# Zoom in on nFeature_RNA violin plot.
VlnPlot(run3.subset, features = "nFeature_RNA", ncol = 1) + ylim(0, 2500) + NoLegend()
...
run3.SCexp <- as.SingleCellExperiment(run3.subset)

# Anotate using raw counts.
SingleR.annotation <- SingleR(test = run3.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, num.threads = detectCores())

run3.subset[["cell.labels"]] <- SingleR.annotation$labels
```

```{r annotation_vis, fig.height=6, fig.width=10}
DimPlot(run3.subset, reduction = "umap", group.by = "cell.labels", label = TRUE, repel = TRUE, label.size = 4) + theme(axis.title = element_text(size = 15), legend.text = element_text(size = 10), axis.text = element_text(size = 10)) + guides(colour = guide_legend(override.aes = list(size = 3)))
```
...
```{r}
clusters <- run3.subset@meta.data[["seurat_clusters"]]
...
SingleR.annotation <- SingleR(test = run3.SCexp, ref = reference, assay.type.test = "logcounts", labels = reference$label.main, clusters = clusters, num.threads = detectCores())

run3.subset[["cluster.labels"]] <- SingleR.annotation$labels
```
...
cluster_number <- levels(run3.subset[["seurat_clusters"]][1, ])
...
names(new.clusters.ids) <- levels(run3.subset)
run3.subset <- RenameIdents(run3.subset, new.clusters.ids)
```

```{r cluster_annotation_vis, fig.height=6, fig.width=10}
DimPlot(run3.subset, reduction = "umap", label = TRUE, repel = TRUE, label.size = 4) + theme(axis.title = element_text(size = 15), legend.text = element_text(size = 10), axis.text = element_text(size = 10)) + guides(colour = guide_legend(override.aes = list(size = 3)))
```

```{r saving_data, include = FALSE, echo = FALSE}

# Save fully-proccesed data.
saveRDS(run3.subset, file = "~/Documents/SC_Prostate/Output/run3/run3_processed.rds")
```
