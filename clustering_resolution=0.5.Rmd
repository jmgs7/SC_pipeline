---
title: "Clustering with resolution = 0.5"
...
SCP_data <- SetIdent(SCP_data, value = SCP_data$RNA_snn_res.0.5)
```
...
clusters <- SCP_data@meta.data[["RNA_snn_res.0.5"]]
...
SCP_data[["cluster.labels_res=0.5"]] <- SingleR.annotation$labels
```
...
cluster_number <- levels(SCP_data[["RNA_snn_res.0.5"]][1, ])
...
SCP_data[["cell.cluster.labels_res=0.5"]] <- Idents(SCP_data)
```
